    ...
    <div class="user-table">
      <div class="d-flex d-fit">
        <div class="d-flex d-start">
          <div class="style">
            <a-input
              :placeholder="$fnc.format('station').searchstation"
              v-model="searchText"
              @pressEnter="onSearch"
        ...
        <div style="">
            <a-button class="custom-btn2" @click="onNewStation" :disabled="!id">{{
...
import StationDrawer from "@/views/pages/Layout/Station/Index.vue";
...
export default {
  ...
  setup() {
    ...
    const onNewStation = () => {
      ...
      state.title = proxy.$fnc.format("station").new;
      state.visible = true;
    };
      ...
      return {
        on: {
          click: (event) => {
            ...
            state.title = proxy.$fnc.format("station").modify;
            state.visible = true;
          },
        },
      };
    ...
    const onApply = async (_obj) => {
      if (state.id) {
        ...
        await proxy.$api.onApiAddStation(state.id, _obj);
        _loading.close();
        state.visible = false;
    ...
    const onEdit = async (_obj) => {
      const _loading = proxy.$vs.loading();
      await proxy.$api.onApiUpdateStation(_obj);
    ...
    const onDelete = async (_obj) => {
      const _robot = state.robots.find((item) => item.charger === _obj.id);

      if (_robot) {
        proxy.$fnc.showError({
          notification: proxy.$vs.notification,
          msg: proxy.$fnc.format("message").msg15,
        });
      } else {
        const _loading = proxy.$vs.loading();
        const _deleteItem = state.map.connections.find(
          (item) =>
            item.source_id === _obj.id || item.destination_id === _obj.id
        );
        if (_deleteItem) {
          await proxy.$api.onApiDeleteConnection(state.id, _deleteItem);
        }
        await proxy.$api.onApiDeleteStation(_obj);
    ...
    const fncQueryStationList = async () => {
      if (state.id) {
        state.data = await proxy.$api.onApiQueryStation(state.id);
